...
mod tests {
    ...
    fn test_per_channel_scaling() -> Result<()> {
        ...
        for (i, &orig) in data.iter().enumerate() {
            ...
            assert!(diff_pt < orig * 0.1, "Per-token mismatch at {}: orig {}, deq {}", i, orig, deq_pt_vec[i]);
            assert!(diff_pc < orig * 0.1, "Per-channel mismatch at {}: orig {}, deq {}", i, orig, deq_pc_vec[i]);
        }
