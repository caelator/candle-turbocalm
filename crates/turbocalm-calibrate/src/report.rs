...
use crate::profiles::{ProfileCollection, ProfileAnalysis, ProfileAnalyzer, ProfileEntry};
...
impl ReportGenerator {
    ...
    fn write_header(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        ...
        writeln!(content)?;
        writeln!(content, "**Generated:** {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC"))?;
        writeln!(content, "**Calibration Run:** {}", collection.metadata.timestamp)?;
    ...
    fn write_executive_summary(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        ...
        writeln!(content)?;
        writeln!(content, "- **Total Profiles Evaluated:** {}", analysis.total_profiles)?;
        writeln!(content, "- **Best Memory Reduction:** {:.1}%", best.fitness.memory_gain * 100.0)?;
        writeln!(content, "- **Quality Preservation:** {:.3} Brier increase, {:.3} cosine penalty",
                 best.fitness.delta_brier_lm, best.fitness.cosine_penalty)?;
        writeln!(content, "- **Optimal Configuration:** {} bits, {} QJL dimensions",
        ...
        writeln!(content, "|--------|-------|")?;
        writeln!(content, "| Search Time | {:.1}s |", collection.metadata.search_stats.search_time_seconds)?;
        writeln!(content, "| Evaluations | {} |", collection.metadata.search_stats.total_evaluations)?;
        writeln!(content, "| Avg Memory Gain | {:.1}% |", analysis.avg_memory_gain * 100.0)?;
        writeln!(content, "| Max Memory Gain | {:.1}% |", analysis.max_memory_gain * 100.0)?;
        writeln!(content, "| Avg Quality Loss | {:.3} |", analysis.avg_quality_degradation)?;
        writeln!(content, "| Min Quality Loss | {:.3} |", analysis.min_quality_degradation)?;
        writeln!(content)?;
    ...
    fn write_search_summary(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        writeln!(content, "## Search Configuration & Results")?;
        writeln!(content)?;

        writeln!(content, "### Dataset Information")?;
        writeln!(content)?;
        writeln!(content, "- **Samples:** {}", collection.metadata.dataset_info.num_samples)?;
        writeln!(content, "- **Avg Sequence Length:** {:.1}", collection.metadata.dataset_info.avg_seq_length)?;
    ...
    fn write_pareto_analysis(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        ...
        writeln!(content)?;
        writeln!(content, "| Rank | Memory Gain (%) | Quality Loss | Bit Width | QJL Dim | Objective |")?;
        writeln!(content, "|------|----------------|---------------|-----------|---------|-----------|")?;
        ...
        if collection.profiles.len() > 10 {
            writeln!(content, "| ... | ... | ... | ... | ... | ... |")?;
            writeln!(content, "*Showing top 10 of {} solutions*", collection.profiles.len())?;
        }
        writeln!(content)?;
    ...
    fn write_best_profiles(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        ...
        // Best memory gain
        let best_memory = collection.profiles.iter()
            .max_by(|a, b| a.fitness.memory_gain.partial_cmp(&b.fitness.memory_gain).unwrap());
        ...
        let best_quality = collection.profiles.iter()
            .min_by(|a, b| {
    ...
    fn write_profile_details(
        ...
        writeln!(content, "- Rotation Seed: {}", profile.profile.rotation_seed)?;
        writeln!(content, "- Clipping Percentile: {:.3}", profile.profile.continuous.clipping_percentile)?;
        writeln!(content, "- Scale Multiplier: {:.2}", profile.profile.continuous.scale_multiplier)?;
        writeln!(content, "- QJL Threshold: {:.1e}", profile.profile.continuous.qjl_threshold)?;
        writeln!(content)?;

        writeln!(content, "**Performance:**")?;
        writeln!(content, "- Memory Reduction: {:.1}%", profile.fitness.memory_gain * 100.0)?;
        writeln!(content, "- Brier Score Increase: {:.4}", profile.fitness.delta_brier_lm)?;
        writeln!(content, "- Cosine Similarity Loss: {:.4}", profile.fitness.cosine_penalty)?;
        writeln!(content, "- Latency Impact: {:.1}%", profile.fitness.latency_penalty * 100.0)?;
        writeln!(content, "- Weighted Objective: {:.4}", profile.objective_value)?;
        writeln!(content)?;
        ...
        writeln!(content, "    rotation_seed: {},", profile.profile.rotation_seed)?;
        writeln!(content, "    continuous: ContinuousParams {{")?;
        writeln!(content, "        clipping_percentile: {},", profile.profile.continuous.clipping_percentile)?;
        writeln!(content, "        scale_multiplier: {},", profile.profile.continuous.scale_multiplier)?;
        writeln!(content, "        qjl_threshold: {},", profile.profile.continuous.qjl_threshold)?;
        writeln!(content, "    }},")?;
        writeln!(content, "}};")?;
        writeln!(content, "```")?;
    ...
    fn write_detailed_profiles(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        ...
        for (i, profile) in collection.profiles.iter().enumerate() {
            ...
            writeln!(content, "| Scale Multiplier | {:.2} |", profile.profile.continuous.scale_multiplier)?;
            writeln!(content, "| QJL Threshold | {:.1e} |", profile.profile.continuous.qjl_threshold)?;
            writeln!(content, "| **Memory Gain** | **{:.1}%** |", profile.fitness.memory_gain * 100.0)?;
            writeln!(content, "| **Quality Loss** | **{:.4}** |", profile.fitness.delta_brier_lm + profile.fitness.cosine_penalty)?;
            writeln!(content, "| **Objective Value** | **{:.4}** |", profile.objective_value)?;
            writeln!(content)?;
        }
    ...
    fn write_recommendations(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        ...
        let dominant_bits = analysis.bit_width_distribution.iter()
            .max_by_key(|(_, &count)| count)
            .map(|(&bits, _)| bits);

        if let Some(bits) = dominant_bits {
            writeln!(content, "- **Preferred Bit Width:** {} bits (appears in {}% of optimal solutions)",
                     bits,
                     analysis.bit_width_distribution[&bits] * 100 / analysis.total_profiles)?;
        }

        // QJL dimension recommendations
        let dominant_qjl = analysis.qjl_dim_distribution.iter()
            .max_by_key(|(_, &count)| count)
            .map(|(&dim, _)| dim);

        if let Some(dim) = dominant_qjl {
            writeln!(content, "- **Preferred QJL Dimension:** {} (appears in {}% of optimal solutions)",
                     dim,
        ...
        writeln!(content)?;
        writeln!(content, "1. **Validation:** Test recommended profiles on held-out validation sets")?;
        writeln!(content, "2. **A/B Testing:** Compare performance in production environment")?;
        writeln!(content, "3. **Monitoring:** Track memory usage and quality metrics post-deployment")?;
        writeln!(content, "4. **Iteration:** Re-run calibration with updated datasets as needed")?;
        writeln!(content)?;
    ...
    fn write_appendix(&self, content: &mut String, collection: &ProfileCollection) -> Result<()> {
        ...
        writeln!(content)?;
        writeln!(content, "This calibration used a two-level evolutionary optimization strategy:")?;
        writeln!(content)?;
        ...
        writeln!(content)?;
        writeln!(content, "```")?;
        writeln!(content, "objective = memory_gain - λ₁·ΔBrierLM - λ₂·cosine_penalty - λ₃·latency_penalty")?;
        writeln!(content, "```")?;
        writeln!(content)?;

        writeln!(content, "### Metrics Definitions")?;
        writeln!(content)?;
        writeln!(content, "- **Memory Gain:** Fractional reduction in model memory footprint")?;
        writeln!(content, "- **ΔBrierLM:** Change in Brier score for language modeling task")?;
        writeln!(content, "- **Cosine Penalty:** 1 - cosine_similarity between original and quantized activations")?;
        writeln!(content, "- **Latency Penalty:** Fractional increase in inference latency")?;
        writeln!(content)?;
...
impl ReportCustomizer {
    ...
    pub fn generate_executive_summary(collection: &ProfileCollection) -> Result<String> {
        ...
        let start_idx = lines.iter().position(|&line| line == "## Executive Summary");
        let end_idx = lines.iter().position(|&line| line.starts_with("## ") && line != "## Executive Summary");
    ...
    pub fn generate_comparison_report(
        ...
        writeln!(content, "| Run | Profiles | Best Memory Gain | Best Quality Loss | Search Time |")?;
        writeln!(content, "|-----|----------|------------------|-------------------|-------------|")?;
...
mod tests {
    ...
    fn test_report_generation() -> Result<()> {
        ...
        assert!(report.content.contains("# TurboCalm Quantization Calibration Report"));
